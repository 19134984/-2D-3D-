module commondata
        implicit none
        ! Grid parameters
        integer(kind=4), parameter :: ny=41, nx=ny*2+1
        ...
        integer(kind=4), parameter :: itc_max = 2000000           ! Maximum iterations

        ! Grid coordinates
        real(kind=8) :: xGrid(1:nx), yGrid(0:ny+1)
...
subroutine bounceback_u()
    ...
    !$acc parallel loop present(f,f_post,ex,ey,xGrid,yGrid)
    do j=1,ny
        do alpha = 1, 8
            !left
            if(alpha == 1 .or. alpha == 5 .or. alpha == 8)then

                delta_x=dble(ex(alpha))*dt0
                delta_y=dble(ey(alpha))*dt0

                f0 = interpolateF(yGrid(inter_y(j,1))+delta_y, yGrid(inter_y(j,2))+delta_y, yGrid(inter_y(j,3))+delta_y&
                    , f_post(nx-2, inter_y(j,1), alpha), f_post(nx-2, inter_y(j,2), alpha)&
                    , f_post(nx-2, inter_y(j,3), alpha), yGrid(j))

                f1 = interpolateF(yGrid(inter_y(j,1))+delta_y, yGrid(inter_y(j,2))+delta_y, yGrid(inter_y(j,3))+delta_y&
                    , f_post(nx-1, inter_y(j,1), alpha), f_post(nx-1, inter_y(j,2), alpha)&
                    , f_post(nx-1, inter_y(j,3), alpha), yGrid(j))

                f2 = interpolateF(yGrid(inter_y(j,1))+delta_y, yGrid(inter_y(j,2))+delta_y, yGrid(inter_y(j,3))+delta_y&
                    , f_post(nx, inter_y(j,1), alpha), f_post(nx, inter_y(j,2), alpha)&
                    , f_post(nx, inter_y(j,3), alpha), yGrid(j))

                f(1, j, alpha) = interpolateF(xGrid(nx-2)+delta_x, xGrid(nx-1)+delta_x, &
                                xGrid(nx)+delta_x, f0, f1, f2, xGrid(nx)+xGrid(2))
            end if
            !right
