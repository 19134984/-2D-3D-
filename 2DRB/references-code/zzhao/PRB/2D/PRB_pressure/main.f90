module commondata
        implicit none
        ! Grid parameters
        integer(kind=4), parameter :: ny=33, nx=ny*10+1
        integer(kind=4), parameter :: nxHalf=(nx-1)/2+1, nyHalf=(ny-1)/2+1

        real(kind=8), parameter :: Ra=5000          !Rayleigh number
        ...
        integer(kind=4), parameter :: B = 10          !aspect ratio
...
subroutine initial()
    ...
    write(*,*)"x=",xGrid(nx+1),"y=",yGrid(ny+1)
    ...
    write(*,*)"_______________________________", 2810950 *dt/((yGrid(ny)+yGrid(1))/Ur)
...
!    write(*,*)itc1
    ...
    do while(itc < itc_max)
       ...
       ! call macro_t()
        ...
        if((MOD(itc,10).EQ.0).and.itc > 2800000) then
            call getT()
        end if
...
!$acc parallel loop private(m,m_post,s,meq,fSource) gang vector collapse(2)
    do j=1,ny
        do i=1,nx
            ...
            Fx(i,j) = 0.0d0
            Fy(i,j) = 0.0d0!gbeta*(temp(i,j)-Tref)*rho(i,j)
...
subroutine bounceback_u()
    ...
    do j = 1, ny
        ...
        u(nx-1,j) = (f(nx-1,j,1)-f(nx-1,j,3)+f(nx-1,j,5)-f(nx-1,j,6)-f(nx-1,j,7)+f(nx-1,j,8)+0.5d0*dt*Fx(nx-1,j))/rho(nx-1,j)
        ...
        f(nx,j,3) = feq(3,rho(nx,j),u(nx,j),v(nx,j))+f(nx-1,j,3)-feq(3,rho(nx-1,j),u(nx-1,j),v(nx-1,j))
        f(nx,j,6) = feq(6,rho(nx,j),u(nx,j),v(nx,j))+f(nx-1,j,6)-feq(6,rho(nx-1,j),u(nx-1,j),v(nx-1,j))
        f(nx,j,7) = feq(7,rho(nx,j),u(nx,j),v(nx,j))+f(nx-1,j,7)-feq(7,rho(nx-1,j),u(nx-1,j),v(nx-1,j))
...
function feq(alpha,rho,u,v)
    ...
    feq = rho*omega*(1.0d0+3.0d0*un+4.5d0*un*un-1.5d0*us2)
