module commondata
        ...
        real(kind=8), parameter :: Ra=1e7                                      ! Rayleigh number
        ...
        real(kind=8), allocatable :: vorticity(:,:)
...
subroutine loadData()
    ...
    open(unit=01,file='MRTcavity-7572000.dat',form="formatted", access="sequential", status='old', iostat=ios)
...
subroutine getVorticity()
    ...
    write(filename,*) int(Ra)
    filename = adjustl(filename)


    open(unit=02,file='vorticity-'//trim(filename)//'.dat',status='unknown')
