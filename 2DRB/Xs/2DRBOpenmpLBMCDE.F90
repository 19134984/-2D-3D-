...
!   Global data
    module commondata
        ...
        character(len=100) :: settingsFile="SimulationSettings2DOpenmpLBMCDE.txt"
        character(len=100) :: pltFile="buoyancyCavity2DOpenmpLBMCDE.plt"
        character(len=100) :: historyFile="NuRe_2DOpenmpLBMCDE.dat"
    ...
    contains
        ...
        pure real(kind=8) function calc_feq(alpha, rhoLoc, uLoc, vLoc)
            ...
            eu = dble(ex(alpha))*uLoc+dble(ey(alpha))*vLoc
            uu = uLoc*uLoc+vLoc*vLoc
            deltaRhoLoc = rhoLoc-rho0
            calc_feq = omega(alpha)*(deltaRhoLoc + &
        ...
        pure real(kind=8) function calc_geq(alpha, TLoc, uLoc, vLoc, rhoLoc)
            ...
            calc_geq = omega(alpha)*TLoc*(1.0d0+eu/cs2+0.5d0*eu*eu/(cs2*cs2)-0.5d0*uu/cs2) + &
                lambdaT(alpha)*TLoc*pressureLoc/(rho0*cs2)
...
!=============================================================
    program main
        ...
        integer(kind=4) :: maxThreads

        call omp_set_num_threads(24)
        maxThreads = omp_get_max_threads()

        call initial(maxThreads)
...
!=============================================================
    subroutine initial(maxThreads)
        use commondata
        implicit none
        integer(kind=4), intent(in) :: maxThreads
        ...
        write(00,*) "2D OpenMP LBM-CDE reproduction"
        write(00,*) "Mesh =", nx, ny
        write(00,*) "Threads =", maxThreads
...
!=============================================================
    subroutine compute_force()
        ...
        !$omp parallel do default(none) shared(Fx,Fy,T,rho,pressure) private(i,j)
        do j = 1, ny
            do i = 1, nx
...
!=============================================================
    subroutine compute_strain_rate()
        ...
        !$omp parallel do default(none) shared(f,rho,u,v,Fx,Fy,Sxx,Sxy,Syy,Sdiv) &
        !$omp& private(i,j,alpha,feqLoc,neqxx,neqxy,neqyy) &
        !$omp& shared(denomDiag,denomShear,denomDiv,coeffTrace)
        do j = 1, ny
            do i = 1, nx
                neqxx = 0.0d0
                neqxy = 0.0d0
                ...
                do alpha = 0, 8
                    ...
                    neqxx = neqxx + dble(ex(alpha)*ex(alpha))*(f(alpha,i,j)-feqLoc)
                    neqxy = neqxy + dble(ex(alpha)*ey(alpha))*(f(alpha,i,j)-feqLoc)
                    neqyy = neqyy + dble(ey(alpha)*ey(alpha))*(f(alpha,i,j)-feqLoc)
                enddo
...
!=============================================================
    subroutine compute_temperature_gradient()
        ...
        !$omp parallel do default(none) shared(g,T,u,v,rho,pressure,Fx,Fy,gradTx,gradTy) &
        !$omp& private(i,j,alpha,geqLoc,neqx,neqy,denom)
        do j = 1, ny
            do i = 1, nx
                neqx = 0.0d0
                neqy = 0.0d0
                do alpha = 0, 8
                    ...
                    neqx = neqx + dble(ex(alpha))*(g(alpha,i,j)-geqLoc)
                    neqy = neqy + dble(ey(alpha))*(g(alpha,i,j)-geqLoc)
                enddo
...
!=============================================================
    subroutine collision()
        ...
        f_post = 0.0d0
        !$omp parallel do default(none) shared(f,f_post,rho,u,v,Fx,Fy,Sxx,Sxy,Syy,Sdiv) &
        !$omp& private(i,j,alpha,eu,eF,uF,feqLoc,Axx,Axy,Ayy,hermite2,phi)
        do j = 1, ny
            do i = 1, nx
                ...
                do alpha = 0, 8
                    ...
                    eF = dble(ex(alpha))*Fx(i,j)+dble(ey(alpha))*Fy(i,j)
                    feqLoc = calc_feq(alpha, rho(i,j), u(i,j), v(i,j))
                    hermite2 = (dble(ex(alpha)*ex(alpha))/cs2-1.0d0)*Axx + &
                        (dble(ey(alpha)*ey(alpha))/cs2-1.0d0)*Ayy + &
                        2.0d0*dble(ex(alpha)*ey(alpha))/cs2*Axy
                    phi = sourcePref*omega(alpha)*(eF/cs2 + eu*eF/(cs2*cs2) - uF/cs2 + rho0*hermite2)
...
!=============================================================
    subroutine collisionT()
        ...
        g_post = 0.0d0
        !$omp parallel do default(none) shared(g,g_post,T,u,v,rho,pressure,Fx,Fy,gradTx,gradTy) &
        !$omp& private(i,j,alpha,geqLoc,scalarSource,psi,eu)
        do j = 1, ny
            do i = 1, nx
                do alpha = 0, 8
                    geqLoc = calc_geq(alpha, T(i,j), u(i,j), v(i,j), rho(i,j))
                    ...
                    scalarSource = heatSourceQ*(1.0d0+eu/cs2) + &
                        (dble(ex(alpha))*(pressure(i,j)*gradTx(i,j)+T(i,j)*Fx(i,j)) + &
                        dble(ey(alpha))*(pressure(i,j)*gradTy(i,j)+T(i,j)*Fy(i,j)))/(rho0*cs2) + &
                        chi_kappa*(dble(ex(alpha))*gradTx(i,j)+dble(ey(alpha))*gradTy(i,j))
                    psi = sourcePref*omega(alpha)*scalarSource
...
!=============================================================
    subroutine streaming()
        ...
        !$omp parallel do default(none) shared(f,f_post) private(i,j,alpha,ip,jp)
        do j = 1, ny
            do i = 1, nx
                do alpha = 0, 8
                    ip = i-ex(alpha)
...
!=============================================================
    subroutine streamingT()
        ...
        !$omp parallel do default(none) shared(g,g_post) private(i,j,alpha,ip,jp)
        do j = 1, ny
            do i = 1, nx
                do alpha = 0, 8
                    ip = i-ex(alpha)
...
!=============================================================
    subroutine bouncebackT()
        ...
        !$omp parallel do default(none) shared(g,g_post,pressure) private(j)
        do j = 1, ny
            g(1,1,j) = -g_post(3,1,j)+2.0d0*omega(1)*Thot + &
                2.0d0*lambdaT(1)*Thot*pressure(1,j)/(rho0*cs2)
            g(5,1,j) = -g_post(7,1,j)+2.0d0*omega(5)*Thot + &
                2.0d0*lambdaT(5)*Thot*pressure(1,j)/(rho0*cs2)
            g(8,1,j) = -g_post(6,1,j)+2.0d0*omega(8)*Thot + &
                2.0d0*lambdaT(8)*Thot*pressure(1,j)/(rho0*cs2)

            g(3,nx,j) = -g_post(1,nx,j)+2.0d0*omega(3)*Tcold + &
                2.0d0*lambdaT(3)*Tcold*pressure(nx,j)/(rho0*cs2)
            g(6,nx,j) = -g_post(8,nx,j)+2.0d0*omega(6)*Tcold + &
                2.0d0*lambdaT(6)*Tcold*pressure(nx,j)/(rho0*cs2)
            g(7,nx,j) = -g_post(5,nx,j)+2.0d0*omega(7)*Tcold + &
                2.0d0*lambdaT(7)*Tcold*pressure(nx,j)/(rho0*cs2)
        enddo
        !$omp end parallel do
#endif
        ...
        g(5,1,1) = -g_post(7,1,1)+2.0d0*omega(5)*Thot + &
            2.0d0*lambdaT(5)*Thot*pressure(1,1)/(rho0*cs2)
        g(8,1,ny) = -g_post(6,1,ny)+2.0d0*omega(8)*Thot + &
            2.0d0*lambdaT(8)*Thot*pressure(1,ny)/(rho0*cs2)
        g(6,nx,1) = -g_post(8,nx,1)+2.0d0*omega(6)*Tcold + &
            2.0d0*lambdaT(6)*Tcold*pressure(nx,1)/(rho0*cs2)
        g(7,nx,ny) = -g_post(5,nx,ny)+2.0d0*omega(7)*Tcold + &
            2.0d0*lambdaT(7)*Tcold*pressure(nx,ny)/(rho0*cs2)
#endif
...
!=============================================================
    subroutine macro()
        ...
        !$omp parallel do default(none) shared(f,rho,u,v,Fx,Fy) private(i,j,momx,momy)
        do j = 1, ny
            do i = 1, nx
...
!=============================================================
    subroutine check()
        ...
        errTSum = 0.0d0
        refTSum = 0.0d0
        !$omp parallel do default(none) shared(u,v,T,up,vp,Tp) private(i,j) &
        !$omp& reduction(+:errUSum,refUSum,errTSum,refTSum)
        do j = 1, ny
            do i = 1, nx
...
!=============================================================
    subroutine output_Tecplot()
        ...
        write(41,'(A)') 'TITLE = "2D OpenMP LBM-CDE side-heated cavity"'
