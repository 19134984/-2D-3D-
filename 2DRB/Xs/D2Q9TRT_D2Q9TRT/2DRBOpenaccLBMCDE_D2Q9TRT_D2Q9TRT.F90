!===================================================================================================
!!! 二维高 Rayleigh 数自然对流：LBM-CDE D2Q9-TRT/D2Q9-TRT（OpenACC 单 GPU）
!!!
...
!!! 离散模型：流场 D2Q9-TRT；温度场采用含 chi_kappa 源项的 LBM-CDE D2Q9-TRT。
!!! 流场奇模态固定按 chi_s 修正后的有效偶松弛尺度满足 magic 3/16。
!!! 温度名义 TRT 率固定为 s_odd=3-sqrt(3)、s_even=4*sqrt(3)-6；chi_kappa 由目标扩散率反算。
!!! 主要论文对应：
...
!!! 固定温度 TRT 点只消除冻结、无源、p=0 D2Q9 体相的四阶项；完整 chi_kappa/压力/力反馈
!!! 以及 halfway BB/ABB 不因此成为全局四阶格式，正式计算仍需独立检查边界层
...
#endif
#if defined(CHI_KAPPA_OVERRIDE) || defined(BASE_TAUG_OVERRIDE)
#error "Fixed D2Q9 thermal TRT rates determine chi_kappa automatically; do not override chi_kappa or tau_g"
#endif
#if defined(ThermalD2Q5) || defined(D2Q5_QUARTIC_RATES) || defined(THERMAL_BGK)
#error "Temperature is fixed to D2Q9-TRT LBM-CDE; D2Q5/Xu/Luo/BGK thermal branches are disabled"
#endif
#define ThermalD2Q9
#define ThermalD2Q9TRT
...
! 全局数据模块
    module commondata
        ...
        !   kappa = sigma_g_odd*(1-chi_kappa)*cT2
        ! 流场沿用 base tau=0.64 或显式 chi_s 的映射。温度场固定名义 TRT 率：
        !   sigma_g_odd=1/sqrt(12), sigma_g_even=1/sqrt(3)
        !   s_g_odd=3-sqrt(3), s_g_even=4*sqrt(3)-6
        ! 这组率满足冻结、无源、p=0 的 D2Q9 四阶完全消除点；目标 kappa 由 chi_kappa 自动匹配。
...
#endif
        real(kind=8), parameter :: sigmaThermalOdd=1.0d0/dsqrt(12.0d0)
        real(kind=8), parameter :: sigmaThermalEven=1.0d0/dsqrt(3.0d0)
        real(kind=8), parameter :: omegaGOdd=1.0d0/(0.5d0+sigmaThermalOdd)
        real(kind=8), parameter :: omegaGEven=1.0d0/(0.5d0+sigmaThermalEven)
        real(kind=8), parameter :: taugL=0.5d0+sigmaThermalOdd ! 梯度闭合沿用名义 odd/flux tau
        real(kind=8), parameter :: chi_kappa=1.0d0-diffusivity/(cT2*sigmaThermalOdd)
        real(kind=8), parameter :: thermalQuarticResidual=8.0d0*sigmaThermalEven*sigmaThermalOdd - &
            4.0d0*sigmaThermalOdd*sigmaThermalOdd-1.0d0
        ...
        real(kind=8), parameter :: sigmaThermalEffective=(1.0d0-chi_kappa)*sigmaThermalOdd
        ...
        character(len=100) :: pltFile="buoyancyCavity2DOpenaccLBMCDE_D2Q9TRT.plt"
        character(len=100) :: historyFile="NuRe_2DOpenaccLBMCDE_D2Q9TRT.dat"
        character(len=100) :: statisticsFile="NuReStatistics_2DOpenaccLBMCDE_D2Q9TRT.dat"
#ifdef unsteadyFlow
        character(len=100) :: snapshotFilePrefix="buoyancyCavity2DOpenaccLBMCDE_D2Q9TRTSnapshot"
        character(len=100) :: snapshotIndexFile="buoyancyCavity2DOpenaccLBMCDE_D2Q9TRTSnapshot-index.dat"
        character(len=100) :: tecplotFilePrefix="buoyancyCavity2DOpenaccLBMCDE_D2Q9TRTTecplot"
#endif
...
!===================================================================================================
    subroutine initial(numAccDevices)
        ...
        if((omegaFOdd.LE.0.0d0).OR.(omegaFOdd.GE.2.0d0)) then
            error stop "The flow TRT odd relaxation rate must lie strictly between zero and two"
        endif
        if((omegaGEven.LE.0.0d0).OR.(omegaGEven.GE.2.0d0).OR. &
            (omegaGOdd.LE.0.0d0).OR.(omegaGOdd.GE.2.0d0)) then
            error stop "The fixed thermal TRT even/odd rates must lie strictly between zero and two"
        endif
        if(abs(thermalQuarticResidual).GT.1.0d-12) then
            error stop "The fixed thermal TRT rates do not satisfy the source-free D2Q9 quartic condition"
        endif
        if(abs(sigmaThermalEffective-diffusivity/cT2).GT.1.0d-12*max(1.0d0,abs(diffusivity/cT2))) then
            error stop "The chi_kappa mapping does not reproduce the target thermal diffusivity"
        endif
        ...
        write(00,*) "Temperature lattice = D2Q9 TRT LBM-CDE with fixed nominal quartic rates"
...
#endif
        write(00,*) "Thermal mapping policy = fixed nominal TRT rates; chi_kappa derived from target diffusivity"
        write(00,*) "viscosity =", real(viscosity,kind=8), "diffusivity =", real(diffusivity,kind=8)
        write(00,*) "taufL =", real(taufL,kind=8), "thermal tau odd/even =", &
            real(0.5d0+sigmaThermalOdd,kind=8), real(0.5d0+sigmaThermalEven,kind=8)
        write(00,*) "tau flow margin from 0.5 =", real(taufL-0.5d0,kind=8)
        ...
        write(00,*) "thermal sigma odd/even/effective =", real(sigmaThermalOdd,kind=8), &
            real(sigmaThermalEven,kind=8), real(sigmaThermalEffective,kind=8)
        write(00,*) "thermal omega odd/even =", real(omegaGOdd,kind=8), real(omegaGEven,kind=8)
        write(00,*) "source-free D2Q9 quartic residual =", real(thermalQuarticResidual,kind=8)
        ...
        write(00,*) "Quartic scope: fixed rates cancel frozen source-free p=0 bulk q4 terms only."
        write(00,*) "Boundary warning: full chi_kappa/pressure/force feedback and halfway BB/ABB need independent checks."
...
!===================================================================================================
! 子程序: collisionT
! 作用: 对原 LBM-CDE D2Q9 温度方程执行 TRT 奇偶碰撞，保留式 (28) 的
! Q、p*grad(T)+T*F 与 chi_kappa*grad(T) 三部分，并分别使用 even/odd 梯形源项前因子。
! 说明: 名义率固定在冻结、无源、p=0 D2Q9 四阶消除点；该性质不外推到完整反馈和边界。
!===================================================================================================
    subroutine collisionT()
        use commondata
        implicit none
        integer(kind=4) :: i, j, alpha, beta, pair
        real(kind=8) :: geqA, geqB, scalarSourceA, scalarSourceB, rawA, rawB
        real(kind=8) :: euA, euB, dEven, dOdd, sourceEven, sourceOdd
        real(kind=8), parameter :: sourcePrefEven=1.0d0-0.5d0*omegaGEven
        real(kind=8), parameter :: sourcePrefOdd=1.0d0-0.5d0*omegaGOdd
        ...
        !$acc& private(i,j,alpha,beta,pair,geqA,geqB,scalarSourceA,scalarSourceB,rawA,rawB, &
        !$acc& euA,euB,dEven,dOdd,sourceEven,sourceOdd)
        do j = 1, ny
            do i = 1, nx
                ! 静止方向是纯 even 模态；式 (28) 中只有均匀热源 Q 在 alpha=0 时非零。
                geqA = calc_geq(0, T(i,j), u(i,j), v(i,j), rho(i,j))
                rawA = tomega(0)*heatSourceQ
                g_post(0,i,j) = g(0,i,j)-omegaGEven*(g(0,i,j)-geqA)+sourcePrefEven*rawA
                ...
                do pair = 1, 4
                    ...
                    geqA = calc_geq(alpha, T(i,j), u(i,j), v(i,j), rho(i,j))
                    geqB = calc_geq(beta, T(i,j), u(i,j), v(i,j), rho(i,j))
                    euA = dble(qex(alpha))*u(i,j)+dble(qey(alpha))*v(i,j)
                    euB = dble(qex(beta))*u(i,j)+dble(qey(beta))*v(i,j)

                    scalarSourceA = heatSourceQ*(1.0d0+euA/cT2) + &
                    ...
                    scalarSourceB = heatSourceQ*(1.0d0+euB/cT2) + &
                        (dble(qex(beta))*(pressure(i,j)*gradTx(i,j)+T(i,j)*Fx(i,j)) + &
                        dble(qey(beta))*(pressure(i,j)*gradTy(i,j)+T(i,j)*Fy(i,j)))/(rho0*cT2) + &
                        chi_kappa*(dble(qex(beta))*gradTx(i,j)+dble(qey(beta))*gradTy(i,j))
                    rawA = tomega(alpha)*scalarSourceA
                    rawB = tomega(beta)*scalarSourceB

                    dEven = 0.5d0*((g(alpha,i,j)-geqA)+(g(beta,i,j)-geqB))
                    dOdd = 0.5d0*((g(alpha,i,j)-geqA)-(g(beta,i,j)-geqB))
                    sourceEven = 0.5d0*(rawA+rawB)
                    sourceOdd = 0.5d0*(rawA-rawB)

                    g_post(alpha,i,j) = g(alpha,i,j)-omegaGEven*dEven-omegaGOdd*dOdd + &
                        sourcePrefEven*sourceEven+sourcePrefOdd*sourceOdd
                    g_post(beta,i,j) = g(beta,i,j)-omegaGEven*dEven+omegaGOdd*dOdd + &
                        sourcePrefEven*sourceEven-sourcePrefOdd*sourceOdd
                enddo
            enddo
        enddo
...
! 边界精度: 这些规则反射的是含完整源项的 TRT 碰撞后分布；固定体相四阶点不自动保证
! halfway BB/ABB 的全局四阶或精确壁面位置，因此仍需独立网格收敛和壁面残差验证。
!===================================================================================================
