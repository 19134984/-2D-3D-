!===================================================================================================
!!! 二维高 Rayleigh 数自然对流：LBM-CDE 流场 + Luo D2Q5 温度场（OpenACC 单 GPU）
!!!
...
!!! 离散模型：流场 D2Q9-TRT；温度场采用 Luo/Wang 矩空间 D2Q5-TRT。
!!! 流场奇模态固定按 chi_s 修正后的有效偶松弛尺度满足 magic 3/16。
!!! 主要论文对应：
!!!   1) 流场平衡态与源项：式 (4)、式 (26)，有效运动黏度见式 (27)；
!!!   2) 温度矩空间：Luo/Wang D2Q5 对流扩散模型，不使用相邻时刻 T*u/T*v 修正；
!!!   3) D2Q5 率满足 Dubois--Lallemand Eq. (55)，冻结纯扩散子域的两个四阶系数同时为零；
!!!   4) 流场使用 halfway BB；温度场使用 D2Q5 halfway ABB/BB；
...
!!! D2Q5 的 halfway BB/ABB 与四阶消差体碰撞并不自动构成全局四阶格式，正式计算仍需独立检查边界层
...
#endif
#if defined(CHI_KAPPA_OVERRIDE) || defined(BASE_TAUG_OVERRIDE) || defined(ThermalD2Q9)
#error "The Luo D2Q5 thermal branch does not use D2Q9 chi_kappa/base-tau_g controls"
#endif
#define ThermalD2Q5
#define D2Q5_QUARTIC_RATES
#ifndef FLOW_ODD_EFFECTIVE_MAGIC
#define FLOW_ODD_EFFECTIVE_MAGIC
#endif
#if defined(FLOW_ODD_UNIT) || defined(FLOW_BGK) || defined(FLOW_ODD_RATE_OVERRIDE)
#error "Direct odd-rate/BGK branches are disabled; the effective flow magic policy is fixed"
#endif
...
! 全局数据模块
    module commondata
        ...
        integer(kind=4), parameter :: thermalQ=4
        ...
        !-------------------------------------------------------------------------------------------
        ! 流场 LBM-CDE 数值自由度与自动映射：nu=(tau_fL-0.5)*(1-chi_s)*cs2。
        ...
        ! Luo/Wang D2Q5 温度参数。M=[1;cx;cy;energy;anisotropy]，平衡 energy=a*T。
        ! 固定通量率 s_k=3-sqrt(3)，再按 Dubois Eq. (55) 计算 s_e/s_nu；当前特殊点代数上
        ! 给出 s_e=s_nu=4*sqrt(3)-6，同时满足 H_k*H_nu=1/6 和完整四阶系数消除。
        real(kind=8), parameter :: thermalRateK=3.0d0-dsqrt(3.0d0)
        real(kind=8), parameter :: thermalSigmaK=1.0d0/thermalRateK-0.5d0
        real(kind=8), parameter :: thermalAlpha=20.0d0*dsqrt(3.0d0)*diffusivity-4.0d0
        real(kind=8), parameter :: thermalCe=(4.0d0+thermalAlpha)/10.0d0
        real(kind=8), parameter :: thermalEqVelocityCoeff=1.0d0/thermalCe
        real(kind=8), parameter :: thermalSigmaNu=1.0d0/(6.0d0*thermalSigmaK)
        real(kind=8), parameter :: thermalSigmaE=thermalSigmaK*(thermalAlpha+4.0d0)/(1.0d0-thermalAlpha) - &
            (2.0d0+3.0d0*thermalAlpha)/(12.0d0*thermalSigmaK*(1.0d0-thermalAlpha))
        real(kind=8), parameter :: thermalRateE=1.0d0/(0.5d0+thermalSigmaE)
        real(kind=8), parameter :: thermalRateNu=1.0d0/(0.5d0+thermalSigmaNu)
        real(kind=8), parameter :: thermalDiffusivityMapped=thermalCe*thermalSigmaK
        real(kind=8), parameter :: thermalIsotropyProduct=thermalSigmaK*thermalSigmaNu
        real(kind=8), parameter :: chi_kappa=0.0d0 ! 仅保留统一输出接口；Luo D2Q5 不使用 LBM-CDE chi_kappa 源项。
        real(kind=8), parameter :: chi_b=CHI_B_OVERRIDE
        real(kind=8), parameter :: rbPerturbation=RB_PERTURBATION_OVERRIDE
        real(kind=8), parameter :: heatSourceQ=0.0d0                  ! 当前方腔无体热源
        ...
        real(kind=8), parameter :: omegaFOdd=1.0d0/(0.5d0+flowMagicParameter/sigmaFlowEffective)
        ...
        real(kind=8), parameter :: sigmaThermalEffective=diffusivity/thermalCe
        ...
        real(kind=8), allocatable :: gradTx(:,:), gradTy(:,:)        ! 仅用于诊断输出的二阶有限差分温度梯度
    ...
    contains
        ...
        ! Luo/Wang D2Q5 权重：w0=(1-a)/5，w1...w4=(4+a)/20。
        pure real(kind=8) function tomega(alpha)
            ...
            select case(alpha)
            case(0)
                tomega = (1.0d0-thermalAlpha)/5.0d0
            case(1,2,3,4)
                tomega = (4.0d0+thermalAlpha)/20.0d0
            case default
                tomega = 0.0d0
            end select
        end function tomega
        ...
        !-------------------------------------------------------------------------------------------
        ! Luo/Wang D2Q5 对流扩散平衡态：geq_i=w_i*T*(1+e_i.u/c_e)。
        pure real(kind=8) function calc_geq(alpha, TLoc, uLoc, vLoc)
            !$acc routine seq
            implicit none
            integer(kind=4), intent(in) :: alpha
            real(kind=8), intent(in) :: TLoc, uLoc, vLoc
            real(kind=8) :: eu

            eu = dble(qex(alpha))*uLoc+dble(qey(alpha))*vLoc
            calc_geq = tomega(alpha)*TLoc*(1.0d0+thermalEqVelocityCoeff*eu)
...
! 主程序：保持“流场一步 -> 温度场一步 -> 定期同步和诊断”的推进顺序。
    program main
        ...
        do while(itc.LT.itc_max)
            ...
            !---------------------------------------------------------------------------------------
            ! 温度场推进：Luo D2Q5 在矩空间碰撞，不加入相邻时刻的 T*u/T*v 差分修正。
            ! 温度碰撞不再依赖 LBM-CDE 的压力、力或局部非平衡温度梯度源项。
            call collisionT()
            call streamingT()
            call bouncebackT()
            call macroT()
...
!===================================================================================================
    subroutine initial(numAccDevices)
        ...
        if((chi_s.GE.1.0d0).OR.(chi_b.GE.1.0d0)) then
            error stop "chi_s and chi_b must be smaller than one"
        endif
        if(taufL.LE.0.5d0) then
            error stop "The mapped shear relaxation time must be larger than 0.5"
        endif
        ...
        if((nx.LT.3).OR.(ny.LT.3)) then
            error stop "The D2Q5 diagnostic gradients require nx and ny to be at least three"
        endif
        if((thermalAlpha.LE.-4.0d0).OR.(thermalAlpha.GE.1.0d0)) then
            error stop "Luo D2Q5 requires -4 < thermalAlpha < 1"
        endif
        if((thermalRateK.LE.0.0d0).OR.(thermalRateK.GE.2.0d0).OR. &
            (thermalRateE.LE.0.0d0).OR.(thermalRateE.GE.2.0d0).OR. &
            (thermalRateNu.LE.0.0d0).OR.(thermalRateNu.GE.2.0d0)) then
            error stop "All Luo D2Q5 nonconserved relaxation rates must lie strictly between zero and two"
        endif
        if(abs(thermalDiffusivityMapped-diffusivity).GT.100.0d0*epsilon(1.0d0)*max(1.0d0,diffusivity)) then
            error stop "Luo D2Q5 diffusivity mapping is inconsistent"
        endif
        if(abs(thermalIsotropyProduct-1.0d0/6.0d0).GT.100.0d0*epsilon(1.0d0)) then
            error stop "Luo D2Q5 isotropy product is inconsistent"
        endif
#ifdef unsteadyFlow
        ...
        do j = 1, ny
            do i = 1, nx
                ...
                enddo
                do alpha = 0, thermalQ
                    g(alpha,i,j) = calc_geq(alpha, T(i,j), u(i,j), v(i,j))
                enddo
            enddo
        enddo
        ...
        write(00,*) "Temperature lattice = D2Q5 TRT Luo/Wang with Dubois Eq. (55) bulk quartic rates"
        ...
        write(00,*) "chi_s =", real(chi_s,kind=8), "chi_b =", real(chi_b,kind=8)
        write(00,*) "chi_kappa =", real(chi_kappa,kind=8), "(report-only; unused by Luo D2Q5)"
#ifdef CHI_S_OVERRIDE
...
#endif
        write(00,*) "Thermal mapping policy = a=20*sqrt(3)*kappa-4; fixed s_k; Eq. (55) s_e/s_nu"
        write(00,*) "viscosity =", real(viscosity,kind=8), "diffusivity =", real(diffusivity,kind=8)
        write(00,*) "taufL =", real(taufL,kind=8), "flow tau margin from 0.5 =", real(taufL-0.5d0,kind=8)
        ...
        write(00,*) "sigma flow effective/odd =", real(sigmaFlowEffective,kind=8), &
            real(sigmaFlowOdd,kind=8)
        write(00,*) "TRT effective magic target/actual =", real(flowMagicParameter,kind=8), &
            real(magicProductEffective,kind=8)
        write(00,*) "thermal a/c_e =", real(thermalAlpha,kind=8), real(thermalCe,kind=8)
        write(00,*) "thermal rates s_k/s_e/s_nu =", real(thermalRateK,kind=8), &
            real(thermalRateE,kind=8), real(thermalRateNu,kind=8)
        write(00,*) "thermal shifts H_k/H_e/H_nu =", real(thermalSigmaK,kind=8), &
            real(thermalSigmaE,kind=8), real(thermalSigmaNu,kind=8)
        write(00,*) "thermal mapped/target diffusivity =", real(thermalDiffusivityMapped,kind=8), &
            real(diffusivity,kind=8)
        write(00,*) "thermal H_k*H_nu / target =", real(thermalIsotropyProduct,kind=8), real(1.0d0/6.0d0,kind=8)
        ...
        write(00,*) "nu/kappa/gBeta are derived from Ra/Pr/Ma/H_LB; flow base-tau policy determines chi_s."
        write(00,*) "Quartic statement applies to the frozen pure-diffusion bulk operator; wall/convection accuracy is checked separately."
#ifdef RayleighBenardCell
...
! 作用: 用二阶有限差分计算诊断与 Nu/耗散输出所需的 grad(T)。
! 说明: Luo D2Q5 碰撞不依赖该梯度；边缘流体节点使用二阶单边差分，内部使用中心差分。
!===================================================================================================
    subroutine compute_temperature_gradient()
        ...
        !$acc parallel loop collapse(2) present(T,gradTx,gradTy) private(i,j)
        do j = 1, ny
            do i = 1, nx
                if(i.EQ.1) then
                    gradTx(i,j) = 0.5d0*(-3.0d0*T(1,j)+4.0d0*T(2,j)-T(3,j))
                elseif(i.EQ.nx) then
                    gradTx(i,j) = 0.5d0*(3.0d0*T(nx,j)-4.0d0*T(nx-1,j)+T(nx-2,j))
                else
                    gradTx(i,j) = 0.5d0*(T(i+1,j)-T(i-1,j))
                endif

                if(j.EQ.1) then
                    gradTy(i,j) = 0.5d0*(-3.0d0*T(i,1)+4.0d0*T(i,2)-T(i,3))
                elseif(j.EQ.ny) then
                    gradTy(i,j) = 0.5d0*(3.0d0*T(i,ny)-4.0d0*T(i,ny-1)+T(i,ny-2))
                else
                    gradTy(i,j) = 0.5d0*(T(i,j+1)-T(i,j-1))
                endif
            enddo
        enddo
...
!===================================================================================================
! 子程序: collisionT
! 作用: 执行 Luo/Wang D2Q5 矩空间碰撞，不加入相邻时刻 T*u/T*v 差分修正。
! 矩顺序: (T, jx, jy, energy, anisotropy)，平衡态为 (T,T*u,T*v,a*T,0)。
!===================================================================================================
    subroutine collisionT()
        use commondata
        implicit none
        integer(kind=4) :: i, j, alpha
        real(kind=8) :: n(0:4), neq(0:4), npost(0:4)
        ...
        !$acc parallel loop collapse(2) present(g,g_post,T,u,v) &
        !$acc& private(i,j,alpha,n,neq,npost)
        do j = 1, ny
            do i = 1, nx
                ...
                neq(1) = T(i,j)*u(i,j)
                neq(2) = T(i,j)*v(i,j)
                neq(3) = thermalAlpha*T(i,j)
                neq(4) = 0.0d0

                npost(0) = n(0)
                npost(1) = n(1)-thermalRateK*(n(1)-neq(1))
                npost(2) = n(2)-thermalRateK*(n(2)-neq(2))
                npost(3) = n(3)-thermalRateE*(n(3)-neq(3))
                npost(4) = n(4)-thermalRateNu*(n(4)-neq(4))

                g_post(0,i,j) = 0.2d0*npost(0)-0.2d0*npost(3)
                g_post(1,i,j) = 0.2d0*npost(0)+0.5d0*npost(1)+0.05d0*npost(3)+0.25d0*npost(4)
                g_post(2,i,j) = 0.2d0*npost(0)+0.5d0*npost(2)+0.05d0*npost(3)-0.25d0*npost(4)
                g_post(3,i,j) = 0.2d0*npost(0)-0.5d0*npost(1)+0.05d0*npost(3)+0.25d0*npost(4)
                g_post(4,i,j) = 0.2d0*npost(0)-0.5d0*npost(2)+0.05d0*npost(3)-0.25d0*npost(4)
            enddo
        enddo
...
!===================================================================================================
! 子程序: bouncebackT
! 作用: Luo D2Q5 恒温壁采用 anti-bounce-back，绝热壁采用 bounce-back。
! 边界精度: 四阶消差结论只属于纯扩散体碰撞；halfway BB/ABB 的壁面误差必须独立验证。
!===================================================================================================
    subroutine bouncebackT()
        ...
        ! Luo D2Q5 恒温 anti-bounce-back：g_in=-g_out_post+2*w_axis*Twall。
