...
module commondata
    ...
    integer(kind=4), parameter :: nx = 81, ny = 81
...
subroutine bounceback_temp()
    ...
    !$omp parallel do default(none) shared(g,gPost,wTemp) private(j)
    do j = 1, ny
        ! 左右定温：采用与 2DRB 一致的反反弹形式
        g(1,1,j) = -gPost(3,1,j) + 2.0d0 * wTemp(3) * Thot
        g(3,nx,j) = -gPost(1,nx,j) + 2.0d0 * wTemp(1) * Tcold
    end do
    !$omp end parallel do

    !$omp parallel do default(none) shared(g,gPost) private(i)
    do i = 1, nx
        ! 上下绝热：采用与 2DRB 一致的半步长反弹/镜像
        g(2,i,1) = gPost(4,i,1)
        g(4,i,ny) = gPost(2,i,ny)
    end do
    !$omp end parallel do

    ! 角点同时属于侧壁定温和水平绝热边界。
    ! 这里仅在四个角点把 g0 调整到与侧壁定温一致，避免角点温度游离。
    g(0,1,1) = Thot - (g(1,1,1) + g(2,1,1) + g(3,1,1) + g(4,1,1))
    g(0,1,ny) = Thot - (g(1,1,ny) + g(2,1,ny) + g(3,1,ny) + g(4,1,ny))
    g(0,nx,1) = Tcold - (g(1,nx,1) + g(2,nx,1) + g(3,nx,1) + g(4,nx,1))
    g(0,nx,ny) = Tcold - (g(1,nx,ny) + g(2,nx,ny) + g(3,nx,ny) + g(4,nx,ny))
