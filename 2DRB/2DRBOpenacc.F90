...
!=============================================================
    ...
    do while( ((errorU.GT.epsU).OR.(errorT.GT.epsT)).AND.(itc.LE.itc_max) )   !只要 (errorU > epsU 或 errorT > epsT) 且 itc ≤ itc_max，就继续循环
        ...
        if(MOD(itc,2000).EQ.0) call check()
         ...
         if( MOD(itc, outputIntervalItc).EQ.0 ) then  ! 达到一个输出间隔outputFrequency，就执行一次计算体平均Nu，是瞬时全场平均Nu
             ...
             if( (outputPltFile.EQ.1).AND.(MOD(itc, backupIntervalItc).EQ.0) ) then
                 ...
                 call output_Tecplot()  !plt 输出备份间隔uvT
             endif
#endif
             
#ifdef unsteadyFlow
             if( (outputBinFile.EQ.1).OR.(outputPltFile.EQ.1) ) call update_host_all_2d_openacc()
             if(outputBinFile.EQ.1) then
                     call output_binary()          !bin 输出后处理间隔uvTrho数据
                     if(MOD(itc, backupIntervalItc).EQ.0) call backupData()  !输出备份间隔fg
             endif   
...
!===================================================================================================
! 子程序: update_host_all_2d_openacc
! 作用: 在主机端输出或后处理前，把设备端最新场变量同步回 CPU。
!===================================================================================================
  subroutine update_host_all_2d_openacc()
    use commondata
    implicit none

    !$acc update self(u,v,T,rho,f,g,Fx,Fy,Bx_prev,By_prev)
#ifdef steadyFlow
    !$acc update self(up,vp,Tp)
#endif
  end subroutine update_host_all_2d_openacc
!===================================================================================================
