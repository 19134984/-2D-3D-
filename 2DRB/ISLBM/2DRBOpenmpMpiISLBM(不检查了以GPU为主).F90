...
! 6) 如果后续改成周期速度/温度边界，或改变半步长边界布置，这里和各后处理子程序都需要重新检查。
    ...
    if(allocated(streamInterpIndexX)) deallocate(streamInterpIndexX, streamInterpIndexY, &
          streamInterpWeightX, streamInterpWeightY, streamInterpValidX, streamInterpValidY)
...
!===================================================================================================
! allocate_halo_buffers_mpi 结束: 已完成本子程序对应的计算或数据处理步骤。
!===================================================================================================
...
!===================================================================================================
! deallocate_halo_buffers_mpi 结束: 已完成本子程序对应的计算或数据处理步骤。
!===================================================================================================
...
!===========================================================================================================================
  subroutine initial()
    ...
    !-----------------------------------------------------------------------------------------------
    ! ISLBM节点坐标与积分权重。MPI 版本中 xp/yp/quadWidthX/quadWidthY 仍然保留为全局一维表，
...
!===================================================================================================
! distribute_scalar_field_root_mpi 结束: 已完成本子程序对应的计算或数据处理步骤。
!===================================================================================================
...
!===================================================================================================
! recv_scalar_field_mpi 结束: 已完成本子程序对应的计算或数据处理步骤。
!===================================================================================================
...
!===================================================================================================
! distribute_distribution_field_root_mpi 结束: 已完成本子程序对应的计算或数据处理步骤。
!===================================================================================================
...
!===================================================================================================
! recv_distribution_field_mpi 结束: 已完成本子程序对应的计算或数据处理步骤。
!===================================================================================================
...
!===================================================================================================
  subroutine streaming()                                    !先迁移，再边界处理
    ...
    f = 0.0d0
    !$omp parallel do default(none) &
    !$omp& shared(f,f_post,ex,ey,xLocalCount,yLocalCount,streamInterpValidX,streamInterpValidY) &
    !$omp& shared(streamInterpIndexX,streamInterpIndexY,streamInterpWeightX,streamInterpWeightY) &
...
!===================================================================================================
    ...
    g = 0.0d0
    !$omp parallel do default(none) &
    !$omp& shared(g,g_post,ex,ey,xLocalCount,yLocalCount,streamInterpValidX,streamInterpValidY) &
    !$omp& shared(streamInterpIndexX,streamInterpIndexY,streamInterpWeightX,streamInterpWeightY) &
    !$omp& private(i,j,ip,jp,alpha,ii,jj,value,useInterp)
    do j = 1, yLocalCount
        do i = 1, xLocalCount
...
!===================================================================================================
    ...
    !$omp parallel do default(none) &
    !$omp& shared(u,up,v,vp,T,Tp,quadWidthX,quadWidthY,xLocalCount,yLocalCount,xStartGlobal,yStartGlobal) &
    !$omp& private(i,j,globalI,globalJ,areaWeight) reduction(+:error1,error2,error5,error6)
    do j = 1, yLocalCount
        do i = 1, xLocalCount
            globalI = xStartGlobal + i - 1
            globalJ = yStartGlobal + j - 1
            areaWeight = quadWidthX(globalI)*quadWidthY(globalJ)
            error1 = error1+areaWeight*((u(i,j)-up(i,j))*(u(i,j)-up(i,j))+(v(i,j)-vp(i,j))*(v(i,j)-vp(i,j)))
...
! 作用: 输出 MPI 版本所有快照共用的 ISLBM 非均匀坐标。
! 用途: 由主程序、时间推进或后处理流程按需调用，保持与参考 ISLBM 代码的接口风格一致。
!===================================================================================================
  subroutine output_SnapshotMeshFile()
    ...
    write(meshUnit,'(A)') '# 2D ISLBM nonuniform coordinates shared by all matching MPI snapshot .bin files'
    write(meshUnit,'(A)') '# binary records are U_nd, V_nd, T, rho'
...
!===================================================================================================
  subroutine calNuRe()
    ...
    NuVolAvg_temp = 0.0d0    
#ifdef SideHeatedCell  
    !$omp parallel do default(none) shared(u,T,quadWidthX,quadWidthY,xStartGlobal,yStartGlobal,xLocalCount,yLocalCount) &
...
#ifdef RayleighBenardCell  
    !$omp parallel do default(none) shared(v,T,quadWidthX,quadWidthY,xStartGlobal,yStartGlobal,xLocalCount,yLocalCount) &
    !$omp& private(i,j,globalI,globalJ,areaWeight) reduction(+:NuVolAvg_temp)
    do j = 1, yLocalCount
        do i = 1, xLocalCount
            globalI = xStartGlobal + i - 1
    ...
    ReVolAvg_temp = 0.0d0
    !$omp parallel do default(none) shared(u,v,quadWidthX,quadWidthY,xStartGlobal,yStartGlobal,xLocalCount,yLocalCount) &
