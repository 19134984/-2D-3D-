"""Read-only audit of the completed Ra=1e8 multiblock case and its final checkpoint."""
from pathlib import Path
import subprocess,sys,json,re,hashlib
import numpy as np

ROOT=Path(__file__).resolve().parents[2]
CASE=ROOT/'运行脚本/Multiblock_sideheated_20260916_v3/Ra1e8/ratio2/sideheated_N512_20260916_v3'
R=CASE/'results'
...
if '--fetch' in sys.argv:
    ...
    with (R/name).open('wb') as out:
        subprocess.run(ssh+[f'cat /data2/XLLi/Multiblock/Ra1e8/ratio2/sideheated_N512_20260916_v3/results/{name}'],stdout=out,check=True)
...
counters=take('<i4',7)
errors=take('<f8',2)
assert (nx,ny,ratio,left,right,bottom,top)==(512,512,2,64,65,64,65)
assert np.allclose(physics[:3],[1e8,.71,.1],rtol=0,atol=0)
assert int(counters[0])==4580000
blocks=[]
for b in range(nb):
...
refs={'paper_513':{'NuVolAvg':30.3130,'ReVolRMS':542.9352,'Nu_hot':30.3503},
      'paper_extrapolated':{'NuVolAvg':30.2041,'ReVolRMS':536.5456,'Nu_hot':30.2096}}
deviations={name:{k:100*(final[k]-val)/val for k,val in row.items()} for name,row in refs.items()}
report={'job':'6981.master','state':'completed_steady_threshold_passed','fine_steps':int(counters[0]),
