"""Read-only audit of the completed Ra=1e7 multiblock case and its final checkpoint."""
from pathlib import Path
import subprocess,sys,json,re,hashlib
import numpy as np

ROOT=Path(__file__).resolve().parents[2]
CASE=ROOT/'运行脚本/Multiblock_sideheated_20260916_v3/Ra1e7/ratio2/sideheated_N512_20260916_v3'
R=CASE/'results'
...
if '--fetch' in sys.argv:
    ...
    with (R/name).open('wb') as out:
        subprocess.run(ssh+[f'cat /data2/XLLi/Multiblock/Ra1e7/ratio2/sideheated_N512_20260916_v3/results/{name}'],stdout=out,check=True)
...
assert np.allclose(physics[:3],[1e7,.71,.1],rtol=0,atol=0)
assert int(counters[0])==3008000
blocks=[]
for b in range(nb):
...
refs={'paper_513':{'NuVolAvg':16.5245,'ReVolRMS':230.6601,'Nu_hot':16.5461},
      'paper_extrapolated':{'NuVolAvg':16.5110,'ReVolRMS':230.1879,'Nu_hot':16.5132}}
deviations={name:{k:100*(final[k]-val)/val for k,val in row.items()} for name,row in refs.items()}
report={'job':'6980.master','state':'completed_steady_threshold_passed','fine_steps':int(counters[0]),
