"""Submit and inspect the authorized Ra=1e7 multiblock case; credentials via SSHPASS."""
from pathlib import Path
import subprocess,sys,json,hashlib,tarfile,io,re

ROOT=Path(__file__).resolve().parents[2]
REL='Ra1e7/ratio2/sideheated_N512_20260916_v3'
...
'''.replace('__CASE__',REMOTE)
    ...
    command=f'test ! -e {REMOTE}/job.id && test ! -e {REMOTE}/results/run.started && tar -xf - -C {REMOTE} && bash -n {REMOTE}/run.pbs && bash -n {REMOTE}/run.sh && sha256sum {REMOTE}/source/solver.F90'
