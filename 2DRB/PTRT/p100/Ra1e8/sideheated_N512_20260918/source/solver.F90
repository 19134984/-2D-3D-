...
!!!    LBM方法
!!!    P-TRT D2Q9 flow + Luo/Wang D2Q5 thermal MRT
!!!    Parent: ../均匀网格/2DRBOpenacc.F90
!!!    Ref: arXiv:2602.06686v1, Eqs. (3), (17), (24); see README.md for forcing.
!=============================================================
...
!   全局模块
    module commondata
        ...
        ! 无量纲参数
#ifndef NX_OVERRIDE
#define NX_OVERRIDE 512
#endif
#ifndef NY_OVERRIDE
#define NY_OVERRIDE 512
#endif
...
#define RAYLEIGH_OVERRIDE 100000000
#endif
        ...
        ! P-TRT: stress rate Snu, third-order odd rate Sq; fourth-order ghost is removed.
        ! (1/Snu-0.5)*(1/Sq-0.5)=3/16; viscosity=(tauf-0.5)/3 is unchanged.
        ...
        integer(kind=4) :: opposite(0:8)
        data opposite/0, 3, 4, 1, 2, 7, 8, 5, 6/
...
!===========================================================================================================================
  subroutine initial()
    ...
    write(00,*) 'flowScheme = P-TRT D2Q9, third-order equilibrium, fourth-order ghost removed'
    write(00,*) 'flowForcing = conserved-moment Guo/MRT (see PTRT/README.md)'
    write(00,*) 'Snu=',Snu, '; Sq=',Sq, '; magic=',(1.0d0/Snu-0.5d0)*(1.0d0/Sq-0.5d0)
#ifdef EnableLegacyThermalScheme
        ...
        do j = 1,ny
            do i = 1,nx
                ...
                do alpha = 0, 8
                    ...
                    ! Third-order Hermite equilibrium, consistent with the P-TRT collision.
                    f(i,j,alpha) = rho(i,j)*omega(alpha)*(1.0d0+3.0d0*un(alpha) &
                        +4.5d0*un(alpha)*un(alpha)-1.5d0*us2 &
                        +4.5d0*un(alpha)*(un(alpha)*un(alpha)-us2))
                enddo
                do alpha = 0, 4
                    un(alpha) = u(i,j)*ex(alpha)+v(i,j)*ey(alpha) 
...
!===================================================================================================
  subroutine enter_data_2d_openacc()
    ...
    !$acc enter data copyin(xp,yp,ex,ey,opposite,omega,omegaT)
...
!===================================================================================================
  subroutine exit_data_2d_openacc()
    ...
    !$acc exit data delete(xp,yp,ex,ey,opposite,omega,omegaT)
...
!===================================================================================================
  subroutine collision()
    ...
    real(kind=8) :: feq(0:8), h(0:8)
    real(kind=8) :: eu, ef, us2, uf, ghostMoment, ghostPhi, ghostPart
    real(kind=8) :: jneqX, jneqY, firstPart, evenPart, oddPart, forcePart

    ! P-TRT ghost subtraction is the D2Q9 second-order Hermite projection.
    ! Keep the conserved first moment explicitly: macro() uses a half-step force,
    ! and the newly evaluated buoyancy can differ from the preceding step's force.
    ! The force below is Guo/MRT with s_j=0, NOT printed Eq. (6)+(30) of the paper.
    !$acc parallel loop gang vector collapse(2) default(none) &
    !$acc& present(f,f_post,rho,u,v,Fx,Fy,T,ex,ey,opposite,omega) async(1) &
    !$acc& private(alpha,feq,h,eu,ef,us2,uf,ghostMoment,ghostPhi,ghostPart, &
    !$acc& jneqX,jneqY,firstPart,evenPart,oddPart,forcePart)
    do j = 1, ny
        do i = 1, nx
          Fx(i,j) = 0.0d0
          ...
          Fy(i,j) = rho(i,j)*gBeta*(T(i,j)-Tref)+rho(i,j)*B2sigemarho &
          ...
          do alpha = 0, 8
            ...
            ! Eq. (3): cubic Hermite contraction, cs2=1/3.
            feq(alpha) = rho(i,j)*omega(alpha)*(1.0d0+3.0d0*eu+4.5d0*eu*eu &
                -1.5d0*us2+4.5d0*eu*(eu*eu-us2))
            h(alpha) = f(i,j,alpha)-feq(alpha)
            ghostPhi = (dble(ex(alpha)*ex(alpha))-1.0d0/3.0d0) &
                *(dble(ey(alpha)*ey(alpha))-1.0d0/3.0d0)
            ghostMoment = ghostMoment+ghostPhi*h(alpha)
            jneqX = jneqX+dble(ex(alpha))*h(alpha)
            jneqY = jneqY+dble(ey(alpha))*h(alpha)
          enddo
          !$acc loop seq
          do alpha = 0, 8
            eu = dble(ex(alpha))*u(i,j)+dble(ey(alpha))*v(i,j)
            ...
            ghostPhi = (dble(ex(alpha)*ex(alpha))-1.0d0/3.0d0) &
                *(dble(ey(alpha)*ey(alpha))-1.0d0/3.0d0)
            ! Eq. (24): weighted ghost norm = 4*cs^8 = 4/81.
            ghostPart = 20.25d0*omega(alpha)*ghostPhi*ghostMoment
            firstPart = 3.0d0*omega(alpha) &
                *(dble(ex(alpha))*jneqX+dble(ey(alpha))*jneqY)
            evenPart = 0.5d0*(h(alpha)+h(opposite(alpha)))-ghostPart
            oddPart = 0.5d0*(h(alpha)-h(opposite(alpha)))-firstPart
            ! Source mass = 0, momentum = F, stress = (1-Snu/2)*(uF+Fu).
            forcePart = omega(alpha)*(3.0d0*ef+(1.0d0-0.5d0*Snu)*(9.0d0*eu*ef-3.0d0*uf))
            f_post(i,j,alpha) = feq(alpha)+firstPart+(1.0d0-Snu)*evenPart &
                +(1.0d0-Sq)*oddPart+forcePart
          enddo
        enddo
    enddo
    return
