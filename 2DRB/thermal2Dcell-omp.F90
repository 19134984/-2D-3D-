...
!~~temperature B.C.~~
    ...
    module commondata
        ...
        integer(kind=4), parameter :: loadInitFG=0, loadInitRho=0  !! load initial velocity, temperature, f and g from previous simulation
        ...
        integer(kind=4), parameter :: nx=1025, ny=nx     !----Section 1----!网格
        ...
        real(kind=8), parameter :: outputFrequency=100.0d0 !~unit free fall time                            !----Section 3----!
        ...
        real(kind=8), parameter :: epsU=1e-7, epsT=1e-7                     !----Section 3----!
        ...
        !----Section 5----!
        integer(kind=4) :: itc
        integer(kind=4), parameter :: itc_max=200 !~!~dimensionlessTimeMax*int(outputFrequency*timeUnit)
...
!         endif
    ...
    !~ write(00,*) "MLUPS = ", real( dble(nx)*dble(ny)*dble(itc)/(timeEnd-timeStart)/1e6 )
    write(00,*) "Time (OMP) = ", real(timeEnd2-timeStart2), "s"
    write(00,*) "MLUPS (OMP) = ", real( dble(nx)*dble(ny)*dble(itc)/(timeEnd2-timeStart2)/1e6 )
...
open(unit=00,file="SimulationSettings.txt",status='unknown',position='append')
    ...
    elseif(loadInitField.EQ.1) then
        ...
        open(unit=01,file=trim(reloadFilePrefix)//"-"//trim(reloadFileName)//".bin",form="unformatted",access="sequential",status='old')
            if(loadInitFG.EQ.1) then
                write(00,*) "Reloading f and g from file"
                read(01) (((f(alpha,i,j), i=1,nx), j=1,ny), alpha=0,8)
                read(01) (((g(alpha,i,j), i=1,nx), j=1,ny), alpha=0,4)
            else
                write(00,*) "Not reloading f and g from file"  
            endif
            ...
            if(loadInitRho.EQ.1) then
                write(00,*) "Reloading rho from file"
        ...
        do j=1,ny
            do i=1,nx
                ...
                dev = dev+g(0,i,j)+g(1,i,j)+g(2,i,j)+g(3,i,j)+g(4,i,j)-T(i,j)
            enddo
        enddo
...
!*!*Begin subroutine collisionT
    ...
    !$omp parallel do default(none) shared(g,g_post,u,v,T) private(i,j,alpha,n,neq,q,n_post) 
    do j = 1, ny
        do i = 1, nx
...
!*!*Begin subroutine check
    ...
    open(unit=01,file='convergence.plt',status='unknown',position='append')
...
!*!*Begin subroutine calNuRe
    ...
    enddo
    !$omp end parallel do
    NuVolAvg(dimensionlessTime) = NuVolAvg_temp/dble(nx*ny)*lengthUnit/diffusivity+1.0d0
    ...
    enddo
    !$omp end parallel do
    ReVolAvg(dimensionlessTime) = dsqrt(ReVolAvg_temp/dble(nx*ny))*lengthUnit/viscosity
