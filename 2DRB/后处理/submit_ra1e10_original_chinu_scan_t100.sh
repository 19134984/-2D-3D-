...
# 第一套算法 Ra=1e10 原始魔法参数 chi_nu 短扫描。
# chi_nu=0 已有发散基线，本脚本提交 0.1--0.9、0.95、0.99。
# 所有算例使用 Zhang 对应的 2049x2049 网格，从静止初场运行到 100 t_ff。
# node07 和 node05 各自组成 afterany 链；单个算例非数退出不会阻断后续参数。

STAGE=/data2/XLLi/LBMCDE/FLOW-TEST/_staging/20260828_ra1e10_original_chi_t100/latest_local.F90
ROOT=/data2/XLLi/LBMCDE/FLOW-TEST/ORIGINAL/Ra1e10
SOURCE_NAME=2DRBOpenaccLBMCDE_D2Q9TRT_D2Q5LuoTRT.F90
NODE05_DEPEND_AFTER=6336.master
...
make_case() {
    ...
    local job_name=O10C${job_tag}T100
...
nonfinite_population_interval_tff=1.0
flow_odd_interval_tff=0.1
node=${node_name}
