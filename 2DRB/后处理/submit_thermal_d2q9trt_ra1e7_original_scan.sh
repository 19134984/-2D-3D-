...
STAGING=${ROOT}/_staging/20260830_ra1e7_original_chi_equal_scan
MASTER=${STAGING}/2DRBOpenaccLBMCDE_D2Q9TRT_D2Q9TRT.local-master.F90
CAMPAIGN=${ROOT}/ORIGINAL_CHI_EQUAL_SCAN/Ra1e7
...
submit_case() {
    ...
    perl -0pi -e 's/integer\(kind=4\), parameter :: nx=\d+, ny=\d+/integer(kind=4), parameter :: nx=257, ny=257/' "${src}"
    perl -0pi -e 's/real\(kind=8\), parameter :: Rayleigh=[^\r\n]*/real(kind=8), parameter :: Rayleigh=1.0d7/' "${src}"
    ...
    grep -q 'parameter :: nx=257, ny=257' "${src}"
    grep -q 'parameter :: Rayleigh=1.0d7' "${src}"
...

    {
        ...
        printf 'case Ra1e7 nx257 ny257 chi_nu=%s chi_kappa=%s policy=ORIGINAL duration_tf=1400\n' "${chi}" "${chi}"
...
#PBS -o ${case_dir}/results/pbs_original_scan.stdout
#PBS -e ${case_dir}/results/pbs_original_scan.stderr
...
dep05=6511.master
for chi in 0.0 0.2 0.4 0.6 0.8; do
