...
sed -i \
    ...
    -e 's/real(kind=8), parameter :: chi_nu=0.5d0/real(kind=8), parameter :: chi_nu=0.2d0/' \
    -e 's/real(kind=8), parameter :: chi_kappa=0.5d0/real(kind=8), parameter :: chi_kappa=0.2d0/' \
    "${SRC}"
...
grep -Fq 'parameter :: chi_nu=0.2d0' "${SRC}"
grep -Fq 'parameter :: chi_kappa=0.2d0' "${SRC}"
...
case_source_hash=$(sha256sum "${SRC}" | awk '{print $1}')
{
    ...
    printf 'case Ra1e10 nx2048 ny2048 chi_nu=0.2 chi_kappa=0.2 policy=BGK target_tf=1000\n'
...
printf '%s|BGK|0.2|restart=300|target=1000|node=%s|dependency=afterany:%s|%s\n' \
