#!/usr/bin/env bash
# Submit fresh Ra=1e7 original/corrected magic scans to 1400 t_ff.
set -euo pipefail
...
configure_source() {
    ...
    sed -i \
        -e 's@integer(kind=4), parameter :: nx=2048, ny=2048@integer(kind=4), parameter :: nx=257, ny=257@' \
        -e 's@real(kind=8), parameter :: Rayleigh=1.0d10@real(kind=8), parameter :: Rayleigh=1.0d7@' \
    ...
    grep -q 'unsteadyRunDuration=1400.0d0' "${source_file}"
    grep -q 'integer(kind=4), parameter :: loadInitField=0' "${source_file}"
...
prepare_case() {
    local branch=$1
    local chi_label=$2
    local case_dir=${ROOT}/${branch}/Ra1e7/chinu${chi_label}
    ...
    configure_source "${source_file}" "${chi_label}" "${branch}"
    ...
    if [[ "${branch}" == ORIGINAL ]]; then
        job_name="LuoO_R7_c${chi_label}"
    else
        job_name="LuoC_R7_c${chi_label}"
    fi
    {
        printf 'branch=%s\n' "${branch}"
        printf 'Ra=1.0d7\n'
        printf 'grid=257x257\n'
        printf 'chi_nu=%sd0\n' "${chi_label}"
        printf 'unsteady_run_duration_tff=1400\n'
        printf 'complete_history_window_tff=700_to_1400\n'
        printf 'final_statistics_window_tff=1050_to_1400\n'
        printf 'expected_complete_history_samples=701\n'
        printf 'expected_final_statistics_samples=351\n'
