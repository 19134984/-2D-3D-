...
with next(md.glob('*Snapshot-*.bin')).open('rb') as f:
    ...
    for b in range(nb):
        ...
        for a,field in enumerate(ref):
            ...
            num[a]+=np.sum((arr[:,:,a]-interp)**2)*h*h
            denom[a]+=np.sum(interp**2)*h*h
