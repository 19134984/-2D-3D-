...
mpirun -np 4 --hostfile $HOSTFILE -map-by dist -mca rmaps_dist_device mlx5_0 -x UCX_MEMTYPE_CACHE=n -x UCX_RNDV_THRESH=8192  -x CUDA_VISIBLE_DEVICES=4,5,6,7  -x LD_LIBRARY_PATH  -mca pml ucx --mca btl ^vader,tcp,openib,uct -x UCX_IB_GPU_DIRECT_RDMA=1 -x UCX_NET_DEVICES=mlx5_0:1 --mca plm_rsh_agent /opt/batch/agent/tools/dstart ./a.out 2>&1


#mpirun -np 16 --hostfile $HOSTFILE -map-by dist -mca rmaps_dist_device mlx5_0  -x UCX_RNDV_THRESH=8192 -x UCX_LOG_LEVEL=info -x UCX_TLS=rc,cuda_copy,gdr_copy -report-bindings -x CUDA_VISIBLE_DEVICES=4,5,6,7 -x UCX_MEMTYPE_CACHE=n -x LD_LIBRARY_PATH  -mca pml ucx --mca btl ^vader,tcp,openib,uct -x UCX_IB_GPU_DIRECT_RDMA=1 -x UCX_NET_DEVICES=mlx5_0:1 --mca plm_rsh_agent /opt/batch/agent/tools/dstart ./a.out > simulation.log${BATCH_JOB_ID} 2>&1
